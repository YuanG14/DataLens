...
export function InsightPanel({ dataset, analytics }: InsightPanelProps) {
  const { status, insights, isSensitiveDomain, canRegenerate, poolSize, error, generate, regenerate } = useDatasetInsights(dataset, analytics);
    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-8">
      ...
      {status === 'success' && (
        <>
          ...
          {poolSize === 0 && (
            <p className="text-sm text-slate-500 text-center py-6 border-t border-slate-100 mt-2">
              No strong relationships, trends, or group differences were detected in this dataset.
            </p>
          )}
          {isSensitiveDomain && (
            <p className="text-xs text-slate-400 mt-4 pt-4 border-t border-slate-100">
              These insights are descriptive observations generated by deterministic rules from the uploaded dataset and should not be interpreted as medical or clinical advice.
            </p>
          )}
        </>
      )}
